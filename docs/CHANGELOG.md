# Journal des tâches terminées — VTC Togo

Historique complet, daté, de chaque tâche terminée depuis le début du
projet — jamais réécrit ni élagué (contrairement à `docs/STATUS.md`, qui
est un instantané court de l'état actuel). Les tâches en cours ou à
faire vivent dans [`TASKS.md`](TASKS.md) ; les idées non démarrées dans
[`12-roadmap.md`](12-roadmap.md).

Ce fichier a été créé le 6 septembre 2026 en déplaçant ici les 50 tâches
(TASK-001 à TASK-050) qui remplissaient jusque-là `TASKS.md` en entier —
rien n'a été supprimé ni résumé, seulement déplacé (voir
`docs/DECISIONS.md`).
